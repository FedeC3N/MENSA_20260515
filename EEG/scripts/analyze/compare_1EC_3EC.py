...
for current_band in bands_of_interest:
    for current_network in networks_of_interest.keys():
        ...
        # Save figure
        figure_path = os.path.join(config['path']['figures'], 'RMSE_1EC_3EC',f"{current_band}_{current_network}_scatterplot.png")
...
for current_band in bands_of_interest:
    for current_network in networks_of_interest_nick.keys():
        ...
        # Title
        plt.title(f"{current_network} - {current_band}\nCorrelation RMSE vs IG", color='black')

        # Save figure
        figure_path = os.path.join(config['path']['figures'], 'RMSE_1EC_3EC', f"{current_band}_{current_network}_glassbrain.png")
        figure_path = Path(figure_path)
        if not os.path.exists(figure_path.parent):
            os.makedirs(figure_path.parent)
