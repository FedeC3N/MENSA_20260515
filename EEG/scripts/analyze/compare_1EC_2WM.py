...
task_1 = "1EC"
task_2 = "2WM"
analysis_name = f"RMSE_{task_1}_{task_2}"
...
for current_method in connectivity_methods:
    ...
    for current_band in bands_of_interest:
        ...
        for current_network in networks_of_interest.keys():
            ...
            for isub, current_sub in enumerate(sub):
                ...
                current_conn_1, _ = read_connectivity(
                    config,
                    BIDS_1,
                ...
                # ----------------------------
                # Load 2WM connectivity
                # ----------------------------
                BIDS_2 = build_BIDS_object(
                    config,
                    current_sub,
                    current_ses,
                    task_2
                ...
                current_conn_2, _ = read_connectivity(
                    config,
                    BIDS_2,
                    method=current_method,
                    space="source",
                ...
                forward_model = read_forward_model(config, BIDS_2)
                ...
                delta = current_conn_1 - current_conn_2
    ...
    results_network.to_csv(
        output_folder / f"{current_method}_{analysis_name}_results_network.csv",
        index=False
    )

    results_node.to_csv(
        output_folder / f"{current_method}_{analysis_name}_results_node.csv",
        index=False
    )
    ...
    for current_band in bands_of_interest:
        for current_network in networks_of_interest.keys():
            ...
            ax.set_title(
                ...
                f"{task_1} vs {task_2} - {current_band} - {current_network}"
            )
            ax.set_xlabel("IG")
            ax.set_ylabel("RMSE")
    ...
    for current_band in bands_of_interest:
        for current_network in networks_of_interest_nick.keys():
            ...
            plt.title(
                f"{current_method.upper()} - {task_1} vs {task_2}\n"
                f"{current_network} - {current_band} | RMSE vs IG",
                color="black"
            )
