"""
Compare the average connectivity between the resting state previous to the task and the following the task

Federico Ramírez-Toraño
04/05/2026

"""

# Imports
import os
import mne
import numpy
import pandas as pd
import seaborn as sns
from pathlib import Path
from nilearn import plotting
import matplotlib.pyplot as plt
from scipy.stats import pearsonr
from scipy.io import loadmat

from init.init_1EC_3EC import init
from sEEGnal.tools.bids_tools import build_BIDS_object, read_plv, read_forward_model
from sEEGnal.tools.template_tools import label_aal

# To avoid errors when debugging
import matplotlib
matplotlib.use("Qt5Agg")

# Define the parameters of interest
bands_of_interest = ['delta','theta','alpha','low_beta','high_beta','gamma']
networks_of_interest = {
    "DMN": [
        "Frontal_med-lh", "Frontal_med-rh",
        "Frontal_med_orb-lh", "Frontal_med_orb-rh",
        "Frontal_sup_med-lh", "Frontal_sup_med-rh",
        "Frontal_medial_orb-lh", "Frontal_medial_orb-rh",
        "Rectus-lh", "Rectus-rh",
        "Cingulo_post-lh", "Cingulo_post-rh",
        "Precuneus-lh", "Precuneus-rh",
        "Angular-lh", "Angular-rh",
        "Temporal_mid-lh", "Temporal_mid-rh",
        "Temporal_Inf-lh", "Temporal_Inf-rh",
        "Temporal_Pole_mid-lh", "Temporal_Pole_mid-rh",
        "ParaHipocampus-lh", "ParaHipocampus-rh"
    ],

    "SAL": [
        "Insula-lh", "Insula-rh",
        "Cingulo_ant-lh", "Cingulo_ant-rh"
    ],

    "FP": [
        "Frontal_sup-lh", "Frontal_sup-rh",
        "Frontal_sup_orb-lh", "Frontal_sup_orb-rh",
        "Frontal_inf_oper-lh", "Frontal_inf_oper-rh",
        "Frontal_inf_trian-lh", "Frontal_inf_trian-rh",
        "Frontal_inf_orb-lh", "Frontal_inf_orb-rh",
        "Parietal_sup-lh", "Parietal_sup-rh",
        "Parietal_inf-lh", "Parietal_inf-rh",
        "Supramarginal-lh", "Supramarginal-rh",
        "Cingulo_med-lh", "Cingulo_med-rh"
    ]
}
networks_of_interest_nick = {
    "DMN": [
        "lMFG", "rMFG",
        "lMFGo", "rMFGo",
        "lSFGm", "rSFGm",
        "lSFGmo", "rSFGmo",
        "lRectus", "rRectus",
        "lPCC", "rPCC",
        "lPrecu", "rPrecu",
        "lAng", "rAng",
        "lMTG", "rMTG",
        "lITG", "rITG",
        "lTPmid", "rTPmid",
        "lParahip", "rParahip"
    ],

    "SAL": [
        "lInsula", "rInsula",
        "lACC", "rACC"
    ],

    "FP": [
        "lSFG", "rSFG",
        "lSFo", "rSFo",
        "lIFGo", "rIFGo",
        "lIFGt", "rIFGt",
        "lIFGo", "rIFGo",
        "lSPG", "rSPG",
        "lIPG", "rIPG",
        "lSMG", "rSMG",
        "lMCC", "rMCC"
    ]
}

# Init the config
config = init()

# Load the demographics
demographic = pd.read_csv(config['path']['demographic'],sep=';')

# Subjects
sub = demographic['ID'].values
sub = [current_sub.split('\\')[-1][4:] for current_sub in sub]

# Save space for RMSE
results_network = []
results_node = []

# Load the IG
IG = demographic['IG'].values

# For each band
for current_band in bands_of_interest:

    print(f"Working on {current_band}")

    # For each network
    for current_network in networks_of_interest.keys():

        print(f"  Working on {current_network}")

        # Save space for the fc_strength
        RMSE_per_node = numpy.empty((len(sub),len(networks_of_interest[current_network])))
        RMSE_per_node.fill(numpy.nan)

        # Go through each subject
        for isub,current_sub in enumerate(sub):

            # Load 1EC
            config['subsystem'] = 'feature_extraction'
            current_ses = '0'
            current_task = '1EC'
            BIDS = build_BIDS_object(config, current_sub, current_ses, current_task)
            current_plv_1EC,_ = read_plv(config, BIDS, space='source', band_name=current_band)

            # Load 2WM
            current_ses = '0'
            current_task = '2WM'
            BIDS = build_BIDS_object(config, current_sub, current_ses, current_task)
            current_plv_3EC, _ = read_plv(config, BIDS, space='source', band_name=current_band)

            # Get the sources of interest
            config['subsystem'] = 'source_reconstruction'
            forward_model = read_forward_model(config, BIDS)
            mri_head_t = forward_model['mri_head_t']
            head_mri_t = mne.transforms.invert_transform(mri_head_t)

            # Associate each source to atlas regions
            atlas = label_aal(config, forward_model['src'], trans=head_mri_t['trans'])
            labels_index = [i for i,label in enumerate(atlas['label']) if label in networks_of_interest[current_network]]
            source_in_network = numpy.isin(atlas['src_area'], labels_index)

            # Get the values of the current network
            n = atlas['rr'].shape[0]
            iu = numpy.triu_indices(n, k=1)  # (row_idx, col_idx)
            mask_network = (
                    source_in_network[iu[0]] &
                    source_in_network[iu[1]] &
                    (atlas['src_area'][iu[0]] != atlas['src_area'][iu[1]])
            )

            # RMSE
            rmse_network = numpy.sqrt(
                numpy.mean((current_plv_1EC[mask_network] - current_plv_3EC[mask_network]) ** 2)
            )

            results_network.append({
                "subject": current_sub,
                "band": current_band,
                "network": current_network,
                "IG": demographic.loc[isub, "IG"],
                "RMSE": rmse_network
            })

            # RMSE per node
            for inode, current_index in enumerate(labels_index):
                current_label = atlas['label'][current_index]

                seed = atlas['src_area'] == current_index
                target = source_in_network & (atlas['src_area'] != current_index)

                mask_node = (
                        (seed[iu[0]] & target[iu[1]]) |
                        (target[iu[0]] & seed[iu[1]])
                )

                rmse_node = numpy.sqrt(
                    numpy.mean((current_plv_1EC[mask_node] - current_plv_3EC[mask_node]) ** 2)
                )

                RMSE_per_node[isub, inode] = rmse_node

                results_node.append({
                    "subject": current_sub,
                    "band": current_band,
                    "network": current_network,
                    "label": current_label,
                    "IG": demographic.loc[isub, "IG"],
                    "RMSE": rmse_node
                })


results_network = pd.DataFrame(results_network)
results_node = pd.DataFrame(results_node)


#########################################
# Plots
print(f"Plotting...")

# Correlation RMSE - IG for networks
#########################################
for current_band in bands_of_interest:
    for current_network in networks_of_interest.keys():

        # Get the current results
        df = results_network[
            (results_network["band"] == current_band) &
            (results_network["network"] == current_network)
        ].dropna(subset=["IG", "RMSE"])

        # Estimate statistics
        r, p = pearsonr(df["IG"], df["RMSE"])
        r2 = r ** 2

        # Plot
        ax = sns.scatterplot(x="IG", y="RMSE", data=df)
        sns.regplot(x="IG", y="RMSE", data=df, ax=ax, scatter=False)
        ax.set_title(f"RMSE-IG correlation - {current_band} - {current_network}")
        ax.set_xlabel("IG")
        ax.set_ylabel("RMSE")
        ax.text(
            0.05, 0.95,
            f"r = {r:.2f}\nR² = {r2:.2f}\np = {p:.3f}",
            transform=ax.transAxes,
            verticalalignment="top",
            bbox=dict(boxstyle="round", alpha=0.2)
        )

        # Save figure
        figure_path = os.path.join(config['path']['figures'], 'RMSE_1EC_2WM',f"{current_band}_{current_network}_scatterplot.png")
        figure_path = Path(figure_path)
        if not os.path.exists(figure_path.parent):
            os.makedirs(figure_path.parent)
        plt.savefig(figure_path)
        plt.close()


#########################################
# Correlation glass brain
#########################################

matfile = os.path.join(config['path']['template'], "template_AAL.mat")
mat = loadmat(matfile)

coordinates = mat['atlas']['pos'][0][0] * 1000
nicks_nested = mat['atlas']['nick'][0][0]
nicks = [n[0][0] for n in nicks_nested]

for current_band in bands_of_interest:
    for current_network in networks_of_interest_nick.keys():

        coords_net = []
        r_values = []
        labels_net = []

        for iarea in range(len(networks_of_interest_nick[current_network])):

            current_nick = networks_of_interest_nick[current_network][iarea]
            current_label = networks_of_interest[current_network][iarea]

            idx_nick = [i for i, nick in enumerate(nicks) if nick == current_nick]

            idx_nick = idx_nick[0]

            df = results_node[
                (results_node["band"] == current_band) &
                (results_node["network"] == current_network) &
                (results_node["label"] == current_label)
                ].dropna(subset=["IG", "RMSE"])

            r, p = pearsonr(df["IG"], df["RMSE"])

            coords_net.append(coordinates[idx_nick])
            r_values.append(r)
            labels_net.append(f"{current_label}\nr={r:.2f}, p={p:.3f}")

        coords_net = numpy.array(coords_net)
        r_values = numpy.array(r_values)

        # Tamaño por magnitud
        sizes = 50 + 150 * (numpy.abs(r_values) / numpy.max(numpy.abs(r_values)))

        # Figura
        fig = plt.figure(figsize=(10, 10))

        display = plotting.plot_markers(
            node_values=r_values,  # color = correlación
            node_coords=coords_net,
            node_size=sizes,
            node_cmap='coolwarm',  # azul-negativo, rojo-positivo
            node_vmin=-0.3,
            node_vmax=0.3,
            display_mode='ortho',  # sagittal, coronal, axial
            black_bg=False
        )

        # Title
        plt.title(f"{current_network} - {current_band}\nCorrelation RMSE vs IG", color='white')

        # Save figure
        figure_path = os.path.join(config['path']['figures'], 'RMSE_1EC_2WM', f"{current_band}_{current_network}_glassbrain.png")
        figure_path = Path(figure_path)
        if not os.path.exists(figure_path.parent):
            os.makedirs(figure_path.parent)
        plt.savefig(figure_path,dpi=300)
        plt.close()